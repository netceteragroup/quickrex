...
public class OrganizeTestTextsAction extends Action {
  ...
  public OrganizeTestTextsAction() {
    super(""); //$NON-NLS-1$
    this.setText(Messages .getString("views.QuickRExView.organizeTestTextsAction.text")); //$NON-NLS-1$
    this.setToolTipText(Messages .getString("views.QuickRExView.organizeTestTextsAction.tooltip")); //$NON-NLS-1$
    PluginImageRegistry imageRegistry = (PluginImageRegistry) QuickRExPlugin.getDefault().getImageRegistry();
    this.setImageDescriptor(imageRegistry.getDescriptor(PluginImageRegistry.IMG_ORGANIZE_TEXTS));
