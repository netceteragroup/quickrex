...
public class OrganizeREsAction extends Action {
  ...
  public OrganizeREsAction() {
    super(""); //$NON-NLS-1$
    this.setText(Messages.getString("views.QuickRExView.organizeREsAction.text")); //$NON-NLS-1$
    this.setToolTipText(Messages.getString("views.QuickRExView.organizeREsAction.tooltip")); //$NON-NLS-1$
    PluginImageRegistry imageRegistry = (PluginImageRegistry) QuickRExPlugin.getDefault().getImageRegistry();
    this.setImageDescriptor(imageRegistry.getDescriptor(PluginImageRegistry.IMG_ORGANIZE_RES));
